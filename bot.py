...
async def refresh_meeting_report(guild: discord.Guild):
    ...
    try:
        ...
        print("refresh_meeting_report fetch error:", e)
        return

    try:
        await msg.edit(embed=build_meeting_embed(guild))
    except Exception as e:
        print("refresh_meeting_report edit error:", e)
...
def build_meeting_embed(guild: discord.Guild):
    ...
    def safe_add_field(name, value):
        nonlocal current_size

        field_size = len(name) + len(value)

        if current_size + field_size > TOTAL_LIMIT:
            return False
    ...
    for i, chunk in enumerate(chunk_list_safe(present_list)):
        if not safe_add_field(
            f"✅ Присутствовали ({len(present_list)})" if i == 0 else "⠀",
            chunk
        ):
            break
    ...
    for i, chunk in enumerate(chunk_list_safe(absent_list)):
        if not safe_add_field(
            f"❌ Отсутствовали ({len(absent_list)})" if i == 0 else "⠀",
            chunk
        ):
            break
    ...
    if not approved_list:
        safe_add_field("🚫 Отсутствовали с причиной (0)", "—")
    else:
        for i, chunk in enumerate(chunk_list_safe(approved_list)):
            if not safe_add_field(
                f"🚫 Отсутствовали с причиной ({len(approved_list)})" if i == 0 else "⠀",
                chunk
            ):
                break
    ...
    async def on_member_update(
        ...
        if test_role not in before.roles and test_role in after.roles:
            ...
            embed1 = discord.Embed(
                title="👋 Добро пожаловать в Alliance",
                description=(
                    "Поздравляем со вступлением в семью.\n"
                    "Ниже указаны требования для перехода с Test на Junior."
                ),
                color=discord.Color.blue()
            )
            ...
            embed1.add_field(
                ...
                value=(
                    "• На перевод даётся **7 дней**.\n"
                    "• Необходимо участвовать в активности семьи.\n"
                    "• Капты/турниры — полный откат.\n"
                    "• RP-активности — достаточно скриншота."
                ),
                inline=False
            )
            ...
            embed1.add_field(
                name="⚠️ Важно",
                value=(
                    "При отсутствии более 3 дней без уведомления "
                    "будет выдано наказание согласно регламенту семьи."
                ),
                inline=False
            )

            embed2 = discord.Embed(
                title="📈 Требования для перевода",
                color=discord.Color.green()
            )
            ...
            embed2.add_field(
                name="🎯 Арена",
                value=(
                    "• 5 арен на карабинах/сайгах/тяге\n"
                    "• 9000+ урона\n"
                    "• КД от 1.0\n"
                    "• Карта: Ангар-Подъёмник\n"
                    "\n"
                    "**Прогресс:** 0/5"
                ),
                inline=False
            ...
            embed2.add_field(
                ...
                value=(
                    "**Tier 3:**\n"
                    "2 отката + 8 скринов от 10k урона, КД 1.1+\n\n"
                    "**Tier 2:**\n"
                    "2 отката + 8 скринов от 12k урона, КД 1.3+"
                ),
                inline=False
            )
            ...
            embed2.add_field(
                name="❗ Примечания",
                value=(
                    "• RP нельзя заменить ареной.\n"
                    "• Арену можно заменить RP x2.\n"
                    "• Все откаты и скрины загружаются в этот канал.\n"
                    "• Возможны исключения по решению старшего состава."
                ),
                inline=False
            )
