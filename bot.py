...
TEST_ROLE_ID = 1344568061805723658
...
def build_birthdays_embed(guild: discord.Guild) -> discord.Embed:
    ...
    for uid_str, ddmm in sorted(BIRTHDAYS.items(), key=lambda x: _ddmm_to_sortkey(x[1])):
        ...
        for num, (uid_str, ddmm) in enumerate(
            sorted(BIRTHDAYS.items(), key=lambda x: _ddmm_to_sortkey(x[1])),
            start=1
        ):
            uid = int(uid_str)
            member = guild.get_member(uid)
...
class Bot(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not message.content.startswith("удал "):
            return
        ...
        if not ref or not ref.embeds:
            return

        embed = ref.embeds[0]

        if embed.title != "Дни рождения":
            return

        birthdays_sorted = sorted(
            BIRTHDAYS.items(),
            key=lambda x: _ddmm_to_sortkey(x[1])
        )

        if index < 1 or index > len(birthdays_sorted):
            await message.reply("❌ Неверный номер")
            return

        uid, _ = birthdays_sorted[index - 1]

        del BIRTHDAYS[uid]
        save_birthdays()

        await ensure_birthdays_list_message(
            self,
            message.guild
        )
    ...
    async def on_member_update(
        ...
        if test_role not in before.roles and test_role in after.roles:
            ...
            await ticket.send(RULES_TEXT)

            await ticket.send(
                "```"
                "\n1. Арена - 0/5  [скрин]"
                "\n2. Rp - 0/25 [x2 если не можешь выполнить арену] [скрин]"
                "\n\nВажно: Пункт с Rp, нельзя заменить ареной | "
                "Пункт с Ареной можно заменить x2 Rp."
                "\n```"
            )
